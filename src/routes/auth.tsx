...
function AuthPage() {
  ...
  // Prefetch rotas críticas quando a página de auth carrega
  usePrefetchRoutes([
  ...
  const // Prefetch rotas críticas antes de navegar (melhora UX)
    router.preloadRoute({ to: "/_authenticated/dashboard" }).catch(() => { });
  navigate({ to: "/dashboard", replace: true });
} else if (mode === "signup") {
  ...
  // Prefetch rotas críticas antes de navegar
  router.preloadRoute({ to: "/_authenticated/dashboard" }).catch(() => { }
        }
const { error } = await supabase.auth.signUp({
  email: form.email,
